...
/, '');
        ...
        price.textContent = (window.APP_SETTINGS.currency === 'USD' ? '$' : '₱') + (Number(prod.price) || 0).toFixed(2);
    ...
    function renderOrder() {
      ...
      if (order.length === 0) {
        ...
        order.forEach(item => {
          ...
          const price = document.createElement('div');
          price.textContent = (window.APP_SETTINGS.currency === 'USD' ? '$' : '₱') + (item.price * item.qty).toFixed(2);
      ...
      function makeRow(label, value, isTotal=false) {
        ...
        const v = document.createElement('div'); v.className='value'; v.textContent = (window.APP_SETTINGS.currency === 'USD' ? '$' : '₱') + Number(value).toFixed(2);
